...
END AS posicion_recurrencia,
    ...
    LOWER(usu_correo) AS usu_correo
        ...
        WHERE est = 1 
        AND sector_id = ? 
        AND usu_id NOT IN (82,102,103,104,105,110,112)"; //que no traiga al usuario dev ni Zettler, ni Ari ni testMauricio, pruebaDeliveryLider
        $stmt = $conn->prepare($sql);
    ...
    CASE 
        ...
        THEN CONCAT(
            LPAD(FLOOR(ROUND((
                (SUM(DISTINCT CASE WHEN horas_usuarios.es_pm = 0 THEN horas_usuarios.total_segundos ELSE 0 END) / 3600) - dimensionamiento.hs_dimensionadas
            ), 2)), 2, '0'), ':',
            LPAD(ROUND((ROUND((
                (SUM(DISTINCT CASE WHEN horas_usuarios.es_pm = 0 THEN horas_usuarios.total_segundos ELSE 0 END) / 3600) - dimensionamiento.hs_dimensionadas
            ), 2) - FLOOR(ROUND((
    ...
    END AS hs_resto,
    GROUP_CONCAT(
        DISTINCT CASE WHEN horas_usuarios.es_pm = 1 THEN
...
ORDER BY clientes.client_rs";
    ...
    public function getDatosReporteConFiltroPoriDCliente($client_id, $sector_id = null)
    {
        ...
        CASE 
            ...
            ELSE CONCAT(
                LPAD(FLOOR(ROUND((
                    dimensionamiento.hs_dimensionadas - 
                    (SUM(DISTINCT CASE WHEN horas_usuarios.id_pm_calidad IS NULL OR horas_usuarios.id_pm_calidad = 0 THEN horas_usuarios.total_segundos ELSE 0 END) / 3600)
                ), 2)), 2, '0'), ':',
                LPAD(ROUND((ROUND((
                    dimensionamiento.hs_dimensionadas - 
                    (SUM(DISTINCT CASE WHEN horas_usuarios.id_pm_calidad IS NULL OR horas_usuarios.id_pm_calidad = 0 THEN horas_usuarios.total_segundos ELSE 0 END) / 3600)
        ...
        END AS hs_resto,
        CASE 
            WHEN SUM(DISTINCT CASE WHEN horas_usuarios.id_pm_calidad = pm_calidad.id THEN horas_usuarios.total_segundos ELSE 0 END) = 0
            THEN NULL
            ELSE CONCAT(
                tm_usuario_pm.usu_nom, ' ',
                TIME_FORMAT(
    ...
    public function get_horas_extras_x_usu($usu_id, $mes, $anio)
    {
        ...
        $sql = "SELECT 
                ...
                TIME_FORMAT(SEC_TO_TIME(SUM(TIME_TO_SEC(tc.horas_consumidas))), '%H:%i') AS total_dia,
                TIME_FORMAT(SEC_TO_TIME(SUM(TIME_TO_SEC(tc.horas_consumidas)) - 28800), '%H:%i') AS horas_extras
            FROM timesummary_carga tc
            WHERE tc.usu_id = :usu_id
              ...
              AND tc.est = 1
            GROUP BY tc.fecha
            HAVING SUM(TIME_TO_SEC(tc.horas_consumidas)) > 28800
    ...
    public function getProyectosStatus(int $sector_id = 4)
    {
        ...
        if ($sector_id == 4) {
            ...
            LEFT JOIN tm_estados ON proyecto_gestionado.estados_id = tm_estados.estados_id
            WHERE proyecto_gestionado.estados_id IN (1,2)
            GROUP BY 
                clientes.client_id,
                clientes.client_rs,
                tm_categoria.cat_nom,
        ...
        $stmt->execute();
            ...
            LEFT JOIN tm_estados ON proyecto_gestionado.estados_id = tm_estados.estados_id
            WHERE sectores.sector_id=:sector_id AND proyecto_gestionado.estados_id IN (1,2)
            GROUP BY 
                clientes.client_id,
                clientes.client_rs,
                tm_categoria.cat_nom,
