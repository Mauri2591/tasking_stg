...
class Validaciones
{
    ...
    public static function subida_archivo($data)
    {
        ...
        // Ruta segura
        $ruta_archivos = dirname(__DIR__, 3) . "/View/Home/Public/Uploads/Calidad/";
        if (!is_dir($ruta_archivos)) {
            if (!mkdir($ruta_archivos, 0777, true)) {
                error_log("Error: No se pudo crear el directorio $ruta_archivos");
                return null;
            }
        }
        ...
        if (move_uploaded_file($data['tmp_name'], $ruta_final)) {
            ...
            error_log("Error: Falló al mover el archivo a $ruta_final");
            return null;
        }
    }
    ...
    public static function guardar_archivo_descripcion_proyecto($data, $hash_folder)
    {
        ...
        // Carpeta por hash con ruta segura
