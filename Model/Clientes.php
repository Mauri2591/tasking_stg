...
class Clientes extends Conexion
{
    public function get_clientes()
    {
        ...
        $sql = "SELECT clientes.*, tm_pais.pais_nombre AS pais FROM clientes INNER JOIN tm_pais ON clientes.pais_id=tm_pais.pais_id WHERE clientes.est=1";
    ...
    {
        $conn = parent::get_conexion();
        $sql = "SELECT clientes.*,tm_pais.pais_nombre AS pais, tm_pais.pais_id FROM clientes INNER JOIN tm_pais ON clientes.pais_id=tm_pais.pais_id WHERE client_id=? AND clientes.est=1";
        $stmt = $conn->prepare($sql);
        $stmt->bindValue(1, htmlspecialchars($client_id, ENT_QUOTES), PDO::PARAM_INT);
        $stmt->execute();
    ...
    public function update_datos_cliente(string $client_id, string $client_rs, int $pais_id, string $client_cuit, string $client_correo, string $client_tel)
    {
        $conn = parent::get_conexion();
        $sql = "UPDATE clientes SET client_rs=?, pais_id=?, client_cuit=?, client_correo=?, client_tel=? WHERE client_id=? AND est=1";
        $stmt = $conn->prepare($sql);
        $stmt->bindValue(1, $client_rs, PDO::PARAM_STR);
        $stmt->bindValue(2, $pais_id, PDO::PARAM_INT);
        $stmt->bindValue(3, $client_cuit, PDO::PARAM_STR);
        $stmt->bindValue(4, $client_correo, PDO::PARAM_STR);
        $stmt->bindValue(5, $client_tel, PDO::PARAM_STR);
        $stmt->bindValue(6, $client_id, PDO::PARAM_STR);
        $stmt->execute();
    }
...
{
    $conn = parent::get_conexion();
    $sql = "SELECT pais_id, pais_nombre FROM tm_pais ORDER BY pais_nombre ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
