...
?>
    ...
    case 'get_proyectos_delivery':
        ...
        foreach ($datos as $row) {
            ...
            $sub_array[] = '<span type="button" onclick="ver_hosts_delivery('
                . $row['id_proyecto_gestionado']
                . ')">
            <i class="text-secondary fs-18 ri-global-line" title="Ver hosts"></i>
        </span>';
                    ...
                    <div style="display:flex">
                        <h6 id="colaborador_descripcion" class="mb-1"><a>
                                ...
                                <span style="font-weight: 600;"><?php echo $val['usu_nom'] ?></span> <span class="text-muted">(<span id="fecha_descripcion"
                                        class="text-muted fs-12"><?php echo date('d-m-Y H:i', strtotime($val['fech_crea'])) ?></span>)</span></a>
                        </h6>
                        <?php
                        ...
                        if ($val['envio_correo_cliente'] == 'SI') {
                            if ($val['status_envio'] == 'OK') {
                                $icono_correo_enviado = '<i class="ri-mail-send-line fs-17 text-success fw-bold" title="Correo enviado correctamente" style="margin-right:4px; font-size:1rem;"></i>';
                            } elseif ($val['status_envio'] == 'ERROR') {
                                $icono_correo_enviado = '<i class="ri-mail-close-line fs-17 fw-bold" title="Error al enviar correo" style="margin-right:4px; font-size:1rem; color:#dc3545;"></i>';
                            }
                        }
                        if ($tienePermiso):
                        ?>
...
TXT;
    ...
    case 'get_datos_ver_recurrente':
        ...
        foreach ($datos as $key => $val) {
            $html .= "
            <li class='badge border border-primary py-1 bg-light fs-11 text-primary mb-1'>
                $key : $val
            </li><br>";
        }
        $html .= "</ul>";
